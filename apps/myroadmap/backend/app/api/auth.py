...
async def linkedin_login():
    ...
    redirect_uri = os.getenv("LINKEDIN_REDIRECT_URI", "http://localhost:8002/api/auth/linkedin/callback")
...
async def linkedin_callback(code: str, db: Session = Depends(get_db)):
    ...
    from fastapi.responses import RedirectResponse
    return RedirectResponse(url=f"http://localhost:3002/login?token={access_token}")
